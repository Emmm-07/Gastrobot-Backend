...
//For Weight Sensor HX711
  ...
  //PINS [available: 0(remove during boot), 12 (low during boot/startup),17,32-33(ADC), 34-39(Input only) ]
 ...
 */
  ...
  //For Servo
  myServoHorizontal.attach(servoPinHorizontal);
  myServoVertical.attach(servoPinVertical);
  ...
  //For Ultrasonic (isContainerEmpty)
  pinMode(TRIG_PIN, OUTPUT); 
  pinMode(ECHO_PIN, INPUT);
  Serial.println("In Ultrasonic");
...
/*
  Since IsFanOn is READ_WRITE variable, onIsFanOnChange() is
  executed every time a new value is received from IoT Cloud.
*/
  void onIsFanOnChange()  {
    // Add your code here to act upon IsFanOn change
    setLogMessage("isFanOn: " + String(isFanOn));
    if(isFanOn){
...
// Weight sensor value -------------------------------------
  ...
  bool readObstacleValue(){
      ...
      setLogMessage("Distance (cm): "+ String(distance_cm));
...
void sensorReadTask1(void *pvParameters) {
    while (true) {
        ...
        gas_Batt_IsEmpty_Obstacle = "{\"gas\":" + String(gasLevel) + ",\"batt\":" + String(batteryLevel) + ",\"container\":" + String(isContainerEmpty) + ",\"obstacle\":" +String(haveObstacle) +  ",\"isWifiConnected\":" + String(isWifiConnected) +"}"; //Changed
